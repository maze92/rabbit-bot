...
// -----------------------------
  ...
  async function loadGuildConfig() {
    ...
    try {
      ...
      const dashAdminsSelect = document.getElementById('configDashAdmins');
      const dashManagersSelect = document.getElementById('configDashManagers');
      const dashViewersSelect = document.getElementById('configDashViewers');
      ...
      const dap = conf && conf.dashboardAccessProfiles ? conf.dashboardAccessProfiles : {};
      fillRoleMultiSelect(dashAdminsSelect, dap.admins);
      fillRoleMultiSelect(dashManagersSelect, dap.managers);
      fillRoleMultiSelect(dashViewersSelect, dap.viewers);
  ...
  async function confirmDeleteDashboardUser(user) {
    ...
    async function saveGuildConfig() {
      ...
      const dashboardAccessProfiles = {
        admins: readRoleMultiSelect(dashAdminsSelect),
        managers: readRoleMultiSelect(dashManagersSelect),
        viewers: readRoleMultiSelect(dashViewersSelect)
      };
      ...
      clearInlineError(dashAdminsSelect);
      clearInlineError(dashManagersSelect);
      clearInlineError(dashViewersSelect);
      ...
      var allRoleIds = staffRoleIds
        ...
        .concat(dashboardAccessProfiles.admins || [])
        .concat(dashboardAccessProfiles.managers || [])
        .concat(dashboardAccessProfiles.viewers || []);
      ...
      try {
        await apiPost('/guilds/' + encodeURIComponent(state.guildId) + '/config', {
          ...
          dashboardAccessProfiles: dashboardAccessProfiles,
          language: language,
          timezone: timezone,
        });
...
// Logs controls
    ...
    var loginDivider = document.querySelector('#loginScreen .login-divider');
    ...
    function showLogin() {
      ...
      try {
        if (loginForm && loginForm.style && loginForm.style.display === 'none') {
          if (loginDiscordBtn) loginDiscordBtn.focus();
        } else {
    ...
    if (getToken()) {
      ...
      if (loginDiscordBtn && !loginDiscordBtn.dataset.bound) {
        ...
        loginDiscordBtn.addEventListener('click', function () {
          try { window.location.href = '/auth/discord'; } catch (e) {}
        });
      }

      // If OAuth is configured, prefer it and hide legacy username/password UX.
      (async function () {
        try {
          var st = await apiGet('/auth/oauth/status');
          var enabled = !!(st && st.enabled);
          if (enabled) {
            if (loginForm) loginForm.style.display = 'none';
            if (loginDivider) loginDivider.style.display = 'none';
          } else {
            if (loginDiscordBtn) loginDiscordBtn.style.display = 'none';
            if (loginDivider) loginDivider.style.display = 'none';
          }
        } catch (e) {
          // If status fails, keep legacy login visible.
          if (loginDiscordBtn) loginDiscordBtn.style.display = 'none';
          if (loginDivider) loginDivider.style.display = 'none';
        }
        showLogin();
