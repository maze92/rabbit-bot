...
(function () {
  ...
  const I18N = {
    pt: {
      ...
      gamenews_search_placeholder: 'Procurar por nome ou URL do feed',
      ...
      gamenews_btn_toggle_logs: 'Ativar / desativar logs',
      gamenews_logs_status_on: 'Logs ativos',
      gamenews_logs_status_off: 'Logs desativados',
      gamenews_feed_log_channel_label: 'Canal de logs',
      gamenews_logs_select_channel: 'Seleciona um canal de logs antes de ativar os logs.',
    ...
    en: {
      ...
      gamenews_search_placeholder: 'Search by feed name or URL',
      ...
      gamenews_btn_toggle_logs: 'Enable / disable logs',
      gamenews_logs_status_on: 'Logs enabled',
      gamenews_logs_status_off: 'Logs disabled',
      gamenews_feed_log_channel_label: 'Log channel',
      gamenews_logs_select_channel: 'Select a log channel before enabling logs.',
    ...
    try {
      ...
      if (dbInfo && typeof dbInfo.trust === 'number') {
        ...
        if (trustLabelLower.includes('alto') || trustLabelLower.includes('high')) {
          ...
          trustLabelLower.includes('baixo') ||
          trustLabelLower.includes('low')
        ) {
          trustLevelClass = 'low';
        } else {
          // Fallback por valor numérico se não houver label textual
          if (typeof trustVal === 'number') {
            if (trustVal >= 70) {
              trustLevelClass = 'high';
            } else if (trustVal >= 40) {
              trustLevelClass = 'neutral';
            } else {
              trustLevelClass = 'low';
            }
          }
        ...
        html +=
          ...
          escapeHtml(trustLevelClass) +
          '">' +
          escapeHtml(String(trustVal)) +
          '</div>';

        // Próxima penalização estimada (se disponível)
        if (dbInfo.nextPenalty) {
          html +=
            '<div class="user-trust-next">' +
            escapeHtml(t('users_trust_next_penalty_simple_prefix')) +
            ' ' +
            escapeHtml(String(dbInfo.nextPenalty)) +
            '</div>';
        }

        html += '</div>'; // fecha user-trust-main
        html += '</div>'; // fecha history-section user-trust
      }

      // Infrações
      html += '<div class="history-section infractions">';
      html += '<h3>' + escapeHtml(t('users_history_infractions')) + '</h3>';
      if (infractions && infractions.length) {
        html += '<ul>';
        infractions.forEach(function (inf) {
          var reason = inf.reason || inf.type || '-';
          var when = inf.date || inf.createdAt || '';
          html +=
            '<li>' +
            escapeHtml(reason + ' • ' + when) +
            '</li>';
        });
        html += '</ul>';
      } else {
        html += '<div class="empty">' + escapeHtml(t('users_history_none')) + '</div>';
      }
      html += '</div>';

      // Tickets recentes
      html += '<div class="history-section tickets">';
      ...
      if (tickets && tickets.length) {
        ...
        tickets.forEach(function (tk) {
          html +=
            '<li>' +
            escapeHtml((tk.subject || tk.id || 'Ticket') + ' • ' + (tk.status || '')) +
            '</li>';
        });
        html += '</ul>';
      } else {
      ...
      // Escrever o HTML final no painel de detalhe
      detailEl.innerHTML = html;
    } catch (err) {
      console.error('Failed to load user history', err);
      detailEl.innerHTML = '<div class="empty">' + escapeHtml(t('users_history_none')) + '</div>';
    }
  } // fim loadUserHistory

  
  function buildUserHistoryHtml(user, dbInfo, infractions, tickets, counts) {
    let html = '';
...
// -----------------------------
  ...
  function buildGameNewsStatusRowHtml(s, lastSent, statusText) {
    return (
      '<div class="title">' +
  ...
  function renderGameNewsStatus(items) {
    ...
    items.forEach(function (s) {
      ...
      row.innerHTML = buildGameNewsStatusRowHtml(s, lastSent, statusText);
...
function renderGameNewsUI() {
  ...
  let feeds = (state.gamenews && Array.isArray(state.gamenews.feeds))
    ? state.gamenews.feeds
    : [];

  const searchTerm = (state.gamenewsSearch || '').trim().toLowerCase();
  if (searchTerm) {
    feeds = feeds.filter(function (f) {
      const name = (f.name || '').toLowerCase();
      const url = (f.feedUrl || '').toLowerCase();
      return (name && name.indexOf(searchTerm) !== -1) || (url && url.indexOf(searchTerm) !== -1);
    });
  }

  feedsList.innerHTML = '';if (!feeds.length) {
    const empty = document.createElement('div');
    empty.className = 'empty';
    empty.textContent = t('gamenews_editor_empty');
    feedsList.appendChild(empty);
  ...
  if (selectedId && feedIds.indexOf(selectedId) !== -1) {
  ...
  feeds.forEach(function (f) {
    ...
    row.addEventListener('click', function () {
      if (!state.gamenews) state.gamenews = { feeds: [], status: [], selectedFeedId: null, channels: [] };
      state.gamenews.selectedFeedId = id;
      var detailEl2 = document.getElementById('gamenewsDetailPanel');
      if (detailEl2) {
        detailEl2.innerHTML = '';
        var loadingDetail2 = document.createElement('div');
        loadingDetail2.className = 'empty';
        loadingDetail2.textContent = t('gamenews_loading');
        detailEl2.appendChild(loadingDetail2);
      }
      setTimeout(function () {
        renderGameNewsUI();
      }, 0);
  ...
  const hasLogs = !!(selectedFeed.logChannelId);
  const logsStatusLabel = hasLogs ? t('gamenews_logs_status_on') : t('gamenews_logs_status_off');
  ...
  header.innerHTML =
    ...
    '  <span class="badge badge-secondary">' + escapeHtml(logsStatusLabel) + '</span>' +
    '</div>';
  ...
  } else if (status && status.paused) {    bannerText = t('gamenews_banner_paused');
  } else if (status && status.failCount && status.failCount > 0) {
  ...
  form.innerHTML =
    ...
    '  </div>' +
    '  <div class="col">' +
    '    <label>' + escapeHtml(t('gamenews_feed_log_channel_label')) + '</label>' +
    '    <select class="select gn-detail-log-channel"></select>' +
    '  </div>' +
    '</div>' +
    '<div class="row gap" style="margin-top:6px;">' +
    ...
    '  <button type="button" class="btn xs" id="btnGamenewsToggleLogs">' + escapeHtml(t('gamenews_btn_toggle_logs')) + '</button>' +
  ...
  const logChannelSelect = detailEl.querySelector('.gn-detail-log-channel');
  const maxInput = detailEl.querySelector('.gn-detail-max');
  const toggleBtn = detailEl.querySelector('#btnGamenewsToggle');
  const toggleLogsBtn = detailEl.querySelector('#btnGamenewsToggleLogs');
  ...
  function updateFromInputs() {
    ...
    if (logChannelSelect) {
      var draft = (logChannelSelect.value || '').trim();
      target._logChannelDraft = draft || null;
    }
  ...
  if (logChannelSelect) {
    logChannelSelect.addEventListener('change', updateFromInputs);
  }
  if (maxInput) {
  ...
  if (toggleLogsBtn) {
    toggleLogsBtn.addEventListener('click', function () {
      ...
      if (target.logChannelId) {
        ...
        // Turn logs on using draft or selected value
        var chosen = target._logChannelDraft;
        if (!chosen && logChannelSelect) {
          chosen = (logChannelSelect.value || '').trim();
        }
        if (!chosen) {
          toast(t('gamenews_logs_select_channel'));
          return;
        }
        target.logChannelId = chosen;
      }
  ...
  if (removeBtn) {
    removeBtn.addEventListener('click', function () {
      ...
      if (state.gamenews.selectedFeedId === effectiveSelectedId) {
...
function syncCurrentGameNewsDetailToState() {
  ...
  var logChannelSelect = detailEl.querySelector('.gn-detail-log-channel');
  ...
  if (logChannelSelect) {
    var logVal = (logChannelSelect.value || '').trim();
    target._logChannelDraft = logVal || null;
  }
  if (maxInput) {
    if (maxInput.value) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (btnAddGameNewsFeed) {
      btnAddGameNewsFeed.addEventListener('click', function () {
        ...
        state.gamenews.selectedFeedId = tempId;
        renderGameNewsUI();
      });
    }

    var gamenewsSearchInput = document.getElementById('gamenewsSearch');
    if (gamenewsSearchInput) {
      gamenewsSearchInput.addEventListener('input', function () {
        var term = (this.value || '').toLowerCase();
        state.gamenewsSearch = term;
        renderGameNewsUI();
      });
    }

    document.addEventListener('click', function (ev) {
      var btn = ev.target.closest('#btnSaveGameNewsFeeds');
      if (!btn) return;
