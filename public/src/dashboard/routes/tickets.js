...
function registerTicketsRoutes(opts) {
  ...
  // Legacy default support message (matches the historical behavior: channel + fixed message + 🎫 reaction).
  // We keep this on the backend so the dashboard UI does not need to ship editable content.
  const SUPPORT_MESSAGE_DEFAULT = {
    pt: '🎫 Para abrir um ticket, reage a esta mensagem.',
    en: '🎫 Need help? React to this message to open a ticket.'
  };

  async function resolveSupportMessageForGuild(guildId) {
    try {
      const GuildConfig = require('../../database/models/GuildConfig');
      const cfg = await GuildConfig.findOne({ guildId }).lean().exec();
      const lang = (cfg && cfg.language ? String(cfg.language) : 'auto').toLowerCase();
      if (lang === 'en') return SUPPORT_MESSAGE_DEFAULT.en;
      // auto => pt fallback
      return SUPPORT_MESSAGE_DEFAULT.pt;
    } catch {
      return SUPPORT_MESSAGE_DEFAULT.pt;
    }
  }
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const provided = typeof sanitizeText === 'function' ? sanitizeText(raw, { maxLen: 1800 }) : raw.slice(0, 1800);
        const message = provided || (await resolveSupportMessageForGuild(guildId));
