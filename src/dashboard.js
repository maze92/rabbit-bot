...
function isAuthEnabled() {
  // Security: auth is enabled whenever a token is configured.
  // This is not runtime-toggleable via dashboard patches.
...
app.post('/api/mod/warn', requireDashboardAuth, async (req, res) => {
  try {
    ...
    if (!guildId || !userId) return res.status(400).json({ ok: false, error: 'guildId and userId are required' });
    const r = (reason || '').toString().trim() || 'Dashboard warning';

    const { guild, member } = await resolveGuildMember(guildId, userId);
    if (!guild || !member) return res.status(404).json({ ok: false, error: 'User not found in guild' });
    ...
    const warnsTextLog = dbUser?.warnings != null ? `\nWarnings: **${dbUser.warnings}**` : '';
    ...
    await logger(
      ...
      member.user,
      _client.user,
      `Reason: **${r}**${warnsTextLog}${trustTextLog}`,
      guild
    );
...
app.post('/api/mod/mute', requireDashboardAuth, async (req, res) => {
  try {
    ...
    if (durationMs > MAX_TIMEOUT_MS) return res.status(400).json({ ok: false, error: 'Duration too long (max 28d).' });
