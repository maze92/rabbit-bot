...
function isAuthEnabled() {
  ...
  if (!token) {
    DASHBOARD_AUTH_MISCONFIG = true;
    console.error('[Dashboard] dashboard.requireAuth=true mas DASHBOARD_TOKEN não está definido. Bloqueando acesso ao dashboard (fail-closed).');
    return false;
  }
...
function requireDashboardAuth(req, res, next) {
  // Fail-closed: if auth is required but misconfigured (no token), block all access.
  if (DASHBOARD_AUTH_MISCONFIG) {
    return res.status(503).json({ ok: false, error: 'Dashboard auth misconfigured (DASHBOARD_TOKEN missing).' });
  }
...
app.post('/api/guilds/:guildId/config', requireDashboardAuth, rateLimit({ windowMs: 60_000, max: 20, keyPrefix: 'rl:guildConfig:' }), async (req, res) => {
  try {
    ...
    const payload = {
      ...
      dashboardLogChannelId: dashboardLogChannelId || null
    };
...
app.post('/api/gamenews/feeds', requireDashboardAuth, rateLimit({ windowMs: 60_000, max: 20, keyPrefix: 'rl:gamenews:feeds:' }), async (req, res) => {
  try {
    ...
    for (const f of feeds) {
      if (!f) continue;
      const name = (f.name || '').toString().trim() || 'Feed';
      const feedUrl = (f.feedUrl || '').toString().trim();
      const channelId = (f.channelId || '').toString().trim();
...
app.post('/api/tickets/:ticketId/close', requireDashboardAuth, async (req, res) => {
  try {
    ...
    if (!ticketId || !guildId) {
      return res.status(400).json({ ok: false, error: 'ticketId and guildId are required' });
    }

    const ticket = await TicketModel.findOne({ _id: ticketId, guildId });
    if (!ticket) {
...
app.get('/api/audit/config', requireDashboardAuth, async (req, res) => {
  try {
    ...
    const items = await DashboardAuditModel.find(query)
      .sort({ at: -1 })
      .limit(limit)
      .lean();
