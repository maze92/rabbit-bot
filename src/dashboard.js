...
app.get('/api/tickets', requireDashboardAuth, async (req, res) => {
  try {
    ...
    let total = await TicketModel.countDocuments(query);
    let items = await TicketModel.find(query)
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();

    // Fallback: if no tickets found for this guild, try returning the latest tickets globally
    if (!total) {
      const globalTotal = await TicketModel.countDocuments({});
      if (globalTotal) {
        total = globalTotal;
        items = await TicketModel.find({})
          .sort({ createdAt: -1 })
          .limit(limit)
          .lean();
      }
