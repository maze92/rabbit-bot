...
app.get('/api/overview', requireDashboardAuth, async (req, res) => {
  try {
    ...
    const guilds = Array.from(_client.guilds.cache.values());
    ...
    if (Infraction) {
      try {
        ...
        actions24h = await Infraction.countDocuments({ createdAt: { $gte: since } });
      } catch (e) {
        console.warn('[Dashboard] Failed to count infractions for overview:', e?.message || e);
        actions24h = 0;
      }
    }
...
app.post('/api/tickets/:ticketId/reply', requireDashboardAuth, async (req, res) => {
  try {
    ...
    if (!ticketId || !guildId || !content) {
      return res.status(400).json({ ok: false, error: 'ticketId, guildId and content are required' });
    }

    const ticket = await TicketModel.findOne({ _id: ticketId, guildId }).lean();
    if (!ticket) {
      return res.status(404).json({ ok: false, error: 'Ticket not found' });
    }
