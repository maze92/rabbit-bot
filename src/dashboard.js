...
const GuildConfigSchema = z.object({
    ...
    // Dashboard access profiles (role-based dashboard login)
    dashboardAccessProfiles: z
      .object({
        viewers: z.array(z.string().regex(/^\d+$/)).max(100).optional(),
        managers: z.array(z.string().regex(/^\d+$/)).max(100).optional(),
        admins: z.array(z.string().regex(/^\d+$/)).max(100).optional()
      })
      .partial()
      .optional(),
...
async function decodeDashboardToken(rawToken) {
  ...
  // JWT path
  try {
    // Explicit algorithms hardening (avoid alg confusion).
    const payload = jwt.verify(rawToken, JWT_SECRET, { algorithms: ['HS256'] });
    if (!payload) return null;

    // OAuth-issued dashboard tokens are self-contained and do not require a DB lookup.
    // This is a transitional mode while we migrate away from dashboard-local users.
    if (payload.t === 'oauth' && payload.sub) {
      return {
        _id: String(payload.sub),
        role: payload.role || 'MOD',
        username: payload.username || 'discord',
        permissions: (payload.permissions && typeof payload.permissions === 'object') ? payload.permissions : {},
        allowedGuildIds: Array.isArray(payload.allowedGuildIds) ? payload.allowedGuildIds : [],
    ...
    // Legacy dashboard-local users (preferred until OAuth is fully enforced)
