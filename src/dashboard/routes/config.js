  ...
  try {
    ...
    if (!doc) {
      return res.json({
        ...
        config: {
          ...
          dashboardAccessProfiles: { viewers: [], managers: [], admins: [] },
          trust: trustConfig
        }
      });
    }
    ...
    return res.json({
      ...
      config: {
        ...
        dashboardAccessProfiles: {
          viewers: Array.isArray(doc.dashboardAccessProfiles?.viewers) ? doc.dashboardAccessProfiles.viewers : [],
          managers: Array.isArray(doc.dashboardAccessProfiles?.managers) ? doc.dashboardAccessProfiles.managers : [],
          admins: Array.isArray(doc.dashboardAccessProfiles?.admins) ? doc.dashboardAccessProfiles.admins : []
        },
        trust: trustConfig
      }
    });
...
app.post('/api/guilds/:guildId/config', requireDashboardAuth, canEditConfig, guardGuildParam, rateLimit({ windowMs: 20_000, max: 20, keyPrefix: 'rl:guildConfig:' }), async (req, res) => {
  try {
    ...
    const { logChannelId, dashboardLogChannelId, ticketThreadChannelId, staffRoleIds, staffRolesByFeature, dashboardAccessProfiles, language, timezone } = req.body || {};
    ...
    if (dashboardAccessProfiles && typeof dashboardAccessProfiles === 'object') {
      const dap = {};
      ['viewers', 'managers', 'admins'].forEach((k) => {
        if (Array.isArray(dashboardAccessProfiles[k])) {
          dap[k] = dashboardAccessProfiles[k].map((id) => sanitizeId(id)).filter(Boolean);
        }
      });
      payload.dashboardAccessProfiles = dap;
    }
    ...
    const candidate = {
      ...
      dashboardAccessProfiles: payload.dashboardAccessProfiles,
