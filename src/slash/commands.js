...
module.exports = function buildSlashCommands(prefix) {
  ...
  return [
    ...
    new SlashCommandBuilder()
      .setName('history')
      .setDescription('Show recent infractions for a user (staff)')
      .addUserOption((opt) =>
        opt.setName('user').setDescription('Target user').setRequired(false)
      )
      .addIntegerOption((opt) =>
        opt
          .setName('limit')
          .setDescription('Number of records (1-15)')
          .setMinValue(1)
          .setMaxValue(15)
          .setRequired(false)
      ),
    new SlashCommandBuilder()
      .setName('ticket')
      .setDescription('Create a support ticket (staff only)')
      .addStringOption((opt) =>
        opt
          .setName('topic')
          .setDescription('Short description of the issue')
          .setRequired(false)
      ),

    new SlashCommandBuilder()
      .setName('ticketclose')
      .setDescription('Close a ticket (current channel or choose a channel)')
      .addChannelOption((opt) =>
        opt
          .setName('channel')
          .setDescription('Ticket channel to close (staff only)')
          .setRequired(false)
      ),
