...
async function handleInfractionAutomation(opts) {
  try {
    ...
    // Contar quantos WARN o utilizador já tem
    let warnsCount = 0;
    try {
      warnsCount = await Infraction.countDocuments({
        guildId: guild.id,
        userId: user.id,
        type: 'WARN'
      }).exec();
    } catch (err) {
      console.warn('[automation] failed to count WARN infractions:', err?.message || err);
      return;
    }
    ...
    let multiplier = 1;
    try {
      const trustUser = await warningsService.getOrCreateUser(guild.id, user.id);
      const trust = typeof trustUser.trust === 'number' ? trustUser.trust : 0;
