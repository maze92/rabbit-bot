...
async function seedFeedsFromConfig(config) {
  if (!GameNewsFeed) return;
  try {
    const count = await GameNewsFeed.countDocuments({});
    if (count > 0) return;
    ...
    console.log('[GameNews] Seeded GameNewsFeed collection from config.gameNews.sources');
  } catch (err) {
    console.error('[GameNews] Failed to seed GameNewsFeed from config:', err?.message || err);
  }
}

async function getEffectiveFeeds(config) {
  // DB is the single source of truth. If there are no feeds in the database
  // we attempt a one-shot seed from config at startup (see seedFeedsFromConfig).
...
module.exports = async function gameNewsSystem(client, config) {
  try {
    ...
    // Ensure DB has initial feeds seeded from config (one-shot)
    await seedFeedsFromConfig(config);
